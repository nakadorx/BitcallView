...
const PlanCard = ({ name, items, price }: { name: string; items: string[]; price: string }) => {
  return (
    <li key={name} className='cursor-pointer hover:scale-110 transition-all duration-300 h-[13rem]'>
      <div className=' bg-primary bg-opacity-5 p-4 rounded-4xl  w-[25rem]'>
      ...
      <div className='relative left-[21rem] top-[-3.5rem] '>
        <div className='bg-white border-primary border-4 rounded-full h-[5rem] w-[5rem] flex items-center justify-center'>
...
const FlexiblePlansSection = async () => {
  ...
  return (
    <SectionContainer title={['', t('plans.title1'), t('plans.title2'), t('plans.title3')]}>
